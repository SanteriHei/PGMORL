...
def _save_population(dirpath, population):
    # Convert the population into JSON serializable format
    payload = []
    for sample in population.sample_batch:
        avg_objs = sample.objs.tolist()
        sd_objs = sample.std_objs.tolist()
        avg_disc_objs = sample.disc_objs.tolist()
        sd_disc_objs = sample.std_disc_objs.tolist()
        obj = {}
        for i in range(len(avg_objs)):
            obj[f"avg_obj_{i}"] = avg_objs[i]
            obj[f"std_obj_{i}"] = sd_objs[i]
            obj[f"avg_disc_obj_{i}"] = avg_disc_objs[i]
            obj[f"std_disc_obj_{i}"] = sd_disc_objs[i]
        payload.append(obj)

    with open(os.path.join(dirpath, "objs.json"), 'w') as ofstream:
        json.dump(payload, ofstream, indent=4)

def _save_ep(dirpath, ep):
    # Convert the EP into JSON serializable format
    payload = []
...
def run(args):
    ...
    while iteration < total_num_updates:
        ...
        _save_population(population_dir, population)
