...
def eval_policy(args, sample, env, seed):
    ob_rms = sample.env_params["ob_rms"]
    policy = sample.actor_critic
    rewards = np.zeros(args.obj_num)
    disc_rewards = np.zeros(args.obj_num)
    gamma = 1.0
    done = False
    ob, _ = env.reset(seed=args.seed)
    with torch.no_grad():
        while not done:
            if args.ob_rms:
            ...
            ob, _, done, info = env.step(torch.permute(action, (1, 0)).squeeze(1))
            rewards += info['obj']
            disc_rewards +=  gamma * info['obj']
            gamma *= args.gamma
...
def evaluation(args, sample):
    eval_env = StepAPICompatibility(
            gym.make(args.env_name), output_truncation_bool=False
    )
    objs = np.zeros((args.eval_num, args.obj_num))
    disc_objs = np.zeros((args.eval_num, args.obj_num))
    # ob_rms = sample.env_params['ob_rms']
    # policy = sample.actor_critic
    with torch.no_grad():
        for eval_id in range(args.eval_num):
            rewards, disc_rewards = eval_policy(
                    args, sample, eval_env, seed=args.seed + eval_id
            )

            objs[eval_id, :] = rewards
            disc_objs[eval_id, :] = disc_rewards
            ...
            #     ob, _, done, info = eval_env.step(torch.permute(action, (1, 0)).squeeze(1))
            #     objs[eval_id, :] += gamma * info['obj']
            #     if not args.raw:
            #         gamma *= args.gamma
            # print(f"Final gamma {gamma}")
    eval_env.close()
    # objs /= args.eval_num
    avg_objs = objs.mean(axis=0)
    sd_objs = objs.std(axis=0)

    avg_disc_objs = disc_objs.mean(axis=0)
    sd_disc_objs = disc_objs.std(axis=0)
    print(f"Avg returns: {avg_objs} | Disc Avg returns: {avg_disc_objs}")

    return avg_objs, sd_objs, avg_disc_objs, sd_disc_objs
...
def MOPG_worker(args, task_id, task, device, iteration, num_updates, start_time, results_queue, done_event):
    ...
    for j in range(start_iter, final_iter):
        ...
        avg_objs, std_objs, avg_disc_objs, std_disc_objs = evaluation(args, sample)
        sample.objs = avg_objs
        sample.std_objs = std_objs
        sample.disc_objs = avg_disc_objs
        sample.std_disc_objs = std_disc_objs
        offspring_batch.append(sample)

        if args.rl_log_interval > 0 and (j + 1) % args.rl_log_interval == 0 and len(episode_rewards) > 1:
            if task_id == 0:
                tmp_env = StepAPICompatibility(
                        gym.make(args.env_name), output_truncation_bool=False
                )
                returns, disc_returns = eval_policy(args, sample, tmp_env, args.seed)
                tmp_env.close()

                sca_ret = task.scalarization.evaluate(
                        torch.from_numpy(returns).to(torch.float64)
                )
                sca_disc_ret = task.scalarization.evaluate(
                        torch.from_numpy(disc_returns).to(torch.float64)
                )

                total_num_steps = (j + 1) * args.num_processes * args.num_steps
                end = time.time()
                print(
                    ("[RL] Updates {}, num timesteps {}, FPS {}, time {:.2f} "
                     "seconds. Scalarized return {}. sca_disc_ret {}")
                    .format(j + 1, total_num_steps,
                            int(total_num_steps / (end - start_time)),
                            end - start_time, sca_ret, sca_disc_ret))
