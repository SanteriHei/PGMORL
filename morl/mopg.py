...
def MOPG_worker(args, task_id, task, device, iteration, num_updates, start_time, results_queue, done_event):
    ...
    print(f"Starting MOPG worker for task {task_id}")
    ...
    print_info(f"Created {args.num_processes} environments")
    ...
    offspring_batch = []
    print_info("starting iterations")
    start_iter, final_iter = iteration, min(iteration + num_updates, total_num_updates)
    for j in range(start_iter, final_iter):
        torch.manual_seed(j)

        if  j % 100 == 0:
            print_info((f"Worker {task_id} | Starting iteration "
                        f"{j}/{final_iter} ({100*(j/(final_iter - start_iter)):.2f}%)"))
        ...
        for step in range(args.num_steps):
            ...
            with torch.no_grad():
                value, action, action_log_prob, recurrent_hidden_states = actor_critic.act(
                    rollouts.obs[step].double(),
                    rollouts.recurrent_hidden_states[step].double(),
                    rollouts.masks[step].double()
                )
            try: 
        ...
        with torch.no_grad():
            next_value = actor_critic.get_value(
                rollouts.obs[-1].double(),
                rollouts.recurrent_hidden_states[-1].double(),
                rollouts.masks[-1].double()
            ).detach().double()
...
def mopg_worker(
        args, task_id, task, device, iteration, num_updates, start_time
):
    scalarization = task.scalarization
    env_params, actor_critic, agent = (
            task.sample.env_params, task.sample.actor_critic, task.sample.agent
    )
    ...
    envs = make_vec_envs(
            env_name=args.env_name, seed=args.seed,
            num_processes=args.num_envs, gamma=args.gamma, log_dir=None,
            device=device, allow_early_resets=False, obj_rms=args.obj_rms,
            ob_rms = args.ob_rms, context=args.context,
            use_shared_memory=args.use_shared_memory,
            daemonize=args.daemonize
    )
    print_info(f"Created {args.num_envs} environments")
    if env_params['ob_rms'] is not None:
        envs.env.ob_rms = deepcopy(env_params["ob_rms"])
    if env_params['ret_rms'] is not None:
        envs.env.ret_rms = deepcopy(env_params['ret_rms'])
    ...
    for j in range(start_iter, final_iter):
        ...
        for step in range(args.num_steps):
            ...
            try: 
                ...
                print_error(f"Trace: {buffer.getvalue()}")
        ...
        if (
                args.rl_log_interval > 0 and 
                (j + 1) % args.rl_log_interval == 0 and 
    ...
    envs.close()
    return task_id, offspring_batch
