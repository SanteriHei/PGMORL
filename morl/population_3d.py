    ...
    for i in range(len(opt_graph.objs)):
        ...
        if np.all(diff < np.abs(opt_graph.objs[index]) * threshold):
            for next_index in opt_graph.succ[i]:
                ...
                weights_data.append(
                    opt_graph.weights[next_index] / np.sum(opt_graph.weights[next_index]))
...
def predict_hyperbolic(args, opt_graph, index, test_weights):
    ...
    def jac(params, x, y):
        ...
        J[1] = (A * (x - b) * (2. * np.exp(a * (x - b))) /
                ((np.exp(a * (x - b)) + 1) ** 2)) * w

        # df_db = A(-a)(2exp(a(x-b)))/(exp(a(x-b)) + 1)^2
        J[2] = (A * (-a) * (2. * np.exp(a * (x - b))) /
                ((np.exp(a * (x - b)) + 1) ** 2)) * w
    ...
    for dim in range(M):
        ...
        if train_y.size == 0:
            raise RuntimeError(
                    (f"No train-y data! {train_y.shape} | "
                     f"Test weights {test_weights} | "
                     f"Weights data {weights_data} | "
                     f"delta_objs data {delta_objs_data}"
                    )
            )
        ...
        res_robust = least_squares(
            fun, params0, loss='soft_l1', f_scale=f_scale,
            args=(train_x, train_y), jac=jac,
...
class Population:
    def __init__(self, args):
        ...
        self.pbuffer_vec = []
        generate_weights_batch_dfs(
            0, args.obj_num, 0.0, 1.0, 1.0 / (args.pbuffer_num - 1),
            [], self.pbuffer_vec
        )

        for i in range(len(self.pbuffer_vec)):
            self.pbuffer_vec[i] = self.pbuffer_vec[i] / \
                np.linalg.norm(self.pbuffer_vec[i])
    ...
    def prediction_guided_selection(
        ...
        for sample in self.sample_batch:
            ...
            weight_candidates = []
            generate_weights_batch_dfs(
                0, args.obj_num, 0.0, 1.0,
                args.delta_weight / 2.0, [], weight_candidates
            )
            test_weights = []
            ...
            for i in range(len(weight_indices)):
                ...
                angle = np.arccos(np.clip(
                    ...
                    np.linalg.norm(weight_center) / np.linalg.norm(weight),
                    -1.0, 1.0))
            ...
            if len(test_weights) > 0:
                try:
                    ...
                    print_error(
                            f"predict_hyperbolic failed with error {e} at "
                            f"iter {iteration} (ep.sample {ep.sample_batch}, "
                            f"ep.obj {ep.obj_batch})"
                    )
                for i in range(len(test_weights)):
        ...
        for _ in range(N):
            ...
            scalarization.update_weights(
                candidates[best_id]['weight'] /
                np.sum(candidates[best_id]['weight'])
            )
    ...
    def random_selection(self, args, scalarization_template):
        ...
        for _ in range(args.num_tasks):
            ...
            weights = np.random.uniform(
                args.min_weight, args.max_weight, args.obj_num)
